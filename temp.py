...
if menu=="Homepage":
    ...
    if dataset=="Loan Prediction":
        st.warning("""
                   **The Problem**:
        Dream Housing Finance company deals in all kinds of home loans. They have presence across all urban, semi urban and rural areas. Customer first applies for home loan and after that company
validates the customer eligibility for loan. Company wants to automate the loan eligibility process (real time) based on customer detail
...
It is a classification problem, given information about the application we have to predict whether the they'll be to pay the loan or not .We'll start by exploratory data analysis, then preprocessing, and finally we'll be testing different models such as Decision Trees. The data consists of the following rows:
        ...
        st.warning("""
                   **The Problem**:    
                   Access to safe drinking-water is essential to health, a basic human right and a component of effective policy for health protection.The data consists of the following rows:
                   """)
    ...
    def describeStat(df):
        ...
        c_eda1.subheader('Null Variales')
        ...
        if c_eda2.button('Data Preprocessing'):
            ...
            if outlier_problem:
                for col in num_array:
                    ...
                    df[col]=np.clip(df[col], a_min=lower_bound[0], a_max=upper_bound[0])
    ...
    if not os.path.exists('formodel.csv'):
        ...
        #Modeling Part
            ...
            report=classification_report(y_test, y_pred, output_dict=(True))  #creating dataframe from classification report
